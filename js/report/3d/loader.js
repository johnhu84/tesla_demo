...
function initSpecialPoints(rId, firstTimeLoad) {
	var itr = setInterval(function(){
		...
		if(model_already_load) {
			...
			// 过滤异常/正常点
			var specialPointIds = data.map(function(item) {
				return item._name;
			});
			window._normalIds = window._normalIds.filter(function(item) {
				return specialPointIds.indexOf(item) !== -1;
			});
			window._abnormalIds = window._abnormalIds.filter(function(item) {
				return specialPointIds.indexOf(item) !== -1;
			});
			// 是否显示/隐藏异常/正常点
			_toggleAbnormal();
 ...
 * @author nikoohp
 */
function _countPointNum() {
	// 特征点
	var totalNumber = window.rid_2_special_point[window._rid].length;
	// 异常点
	var abnormalNumber = window._abnormalIds.length;
	// 正常点
	var normalNumber = window._normalIds.length;

	var $abnormalElm = $('#abnormal-point-number');
	var $normalElm = $('#normal-point-number');
	$normalElm.text(normalNumber + '/' + totalNumber);
	$abnormalElm.text(abnormalNumber + '/' + totalNumber);
}
 ...
 * @author nikoohp
 */
function _defaultReport() {
	$('#handoverReport').val(reportId);
