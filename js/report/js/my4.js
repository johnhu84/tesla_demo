function labelPositionOptimizerUsingArea2(labelW, labelH, count, x1, y1) {
    ...
    while (labelW2 > minW && labelH2 > minH) {
        ...
        var canvasAreaHelper = labelPositionOptimizerUsingAreaHelper(labelW2, labelH2, count, x1, y1);
        var containerAreaHelper = labelPositionOptimizerUsingAreaHelper2(labelW2, labelH2, count, x1, y1);
        //call helper function here to see this labelW2 and this labelH2 can fit count number of labels
        if (canvasAreaHelper || containerAreaHelper ) {
            _width = labelW2;
            _height = labelH2;
            break;
        }
...
function labelPositionOptimizerUsingAreaHelper2(labelW, labelH, count, x1, y1) {
    ...
    // 模型canvas宽高
    var containerWidth = containerElm.offsetWidth;
    var containerHeight = containerElm.offsetHeight;
    // 模型canvas离画布左侧距离
    var containerLeft = containerElm.offsetLeft;
    // 模型canvas离画布上侧距离
    var containerTop = containerElm.offsetTop;
    ...
    // 上下左右空隙是否可画标签框
    var useTheseSides = {
        up: upSideHeight >= labelH,
        left: leftSideWidth >= labelW,
        right: rightSideWidth >= labelW,
        down: downSideHeight >= labelH
    };
    var startPositions = {
        up: {
...
function moveParetoChartsFromAltToAltHolder() {
    ...
    var helpers = [
        '#ParetoDiagramHelper',
        '#TrendMapDiagramHelper',
        '#ControlChartDiagramHelper',
        '#SigmaMapDiagramHelper',
        '#BoxLineChartDiagramHelper',
        '#BoxLineChartDiagramHelper',
        '#StackingMapDiagramHelper',
        '#PieChartDiagramHelper',
        '#HistogramDiagramHelper'
    ];
...
function setTrendMapAlt(id, data, BigobjHelper, widthHelper2, heightHelper2) {
    ...
    // 公差数组为1时，无法显示图表
    if (tolerance && tolerance.length < 2) {
        tolerance = tolerance.concat(tolerance)
    }
    ...
    for (var i = 0; i < result.length; i++) {//数据格式生成
        item = {};
        item.name = i+1;//result[i].s_label
                ...
                if (BigobjHelper.instructionLines[j].IndicatorLineStyleChange == 1) {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        ...
                        if (startTime < Number(toleranceRule[0].Rule.split("<")[0].replace("=", ""))) {
                            ...
                            var timeItemLow = {};
                            // 与x轴相对应
                            timeItem.x = i + 1;//result[i].s_label
                            timeItem.y = +tolerance1[tolerance1.length - 1].fvlHivalue
                            timeItemLow.x = i + 1;//result[i].s_label
...
// 直方图
function setHistogram(ids, widthHelper, heightHelper, BigobjHelper, flabel, res) {
    result = res.points[0];
    var options = {
        data: [{
            x: -1.5,
...
function getCheckSampleIds() {
    /*var getSelectionRows = $manualSampleTable.bootstrapTable('getSelections', function(row) {
        return row
    })
    var sampleIds = ''
...
function setPareToDiagram(ids, data, widthHelper, heightHelper, BigobjHelper, loadEventHandler) {
    ...
    for (var i = 0; i < data.points[0].sample.length; i++) {//数据
        ...
        if (BigobjHelper && typeof BigobjHelper != String) {
            ...
            // 柱子样式
            if (typeof BigobjHelper.dataStyles !== 'undefined' && BigobjHelper.dataStyles.length > 0) {
    ...
    console.log(ParetoDiagramSet.categories)
