...
function InsertRowBeforeWrapper() {
    //handle muban or non-muban here
    if (yemianOrMubanMode == 2) {
        InsertRowBeforeMuban();
        return false
    } else {
        InsertRowBefore();
        return false
...
function InsertRowBeforeMuban() {
    ...
    if (typeof currSelection === 'undefined' || typeof currSelection[0] === 'undefined'
        ...
        return false;
    var mergeCellHelper = fourCornersHelper(currSelection[0]);
    var handsonTableOptions2 = handsonTable2.getSettings();

    var newRow = mergeCellHelper.topLeft.y;
    handsonTableOptions2.data.splice(newRow, 0, []);
    handsonTableOptions2.rowHeights.splice(newRow, 0, 20);
    handsonTableOptions2.height += 20;
    handsonTableOptions2.startRows++;
    /*var newHandsonTableHelperObj = {};
    //does handsonTableHelperObj need to be saved in localStorage?
    //handsonTableHelperObj = JSON.parse(localStorage.getItem('handsonTableHelperObj'));
    for (var i in handsonTableHelperObj) {
    ...
    var newHandsonTableHelperObjImgs = {}
    for (var i in muban_handsonTableImgs[handsonTableSelected]) {
        var helper = i.split(',');
        if (helper[0] >= newRow) {
            handsonTableOptions2.data[Number(helper[0]) + 1][helper[1]] =
                handsonTableOptions2.data[helper[0]][helper[1]]
            handsonTableOptions2.data[helper[0]][helper[1]] = ''
            newHandsonTableHelperObjImgs[(Number(helper[0]) + 1) + ',' + helper[1]] =
                muban_handsonTableImgs[handsonTableSelected][i];
        } else if (helper[0] < newRow) {
            newHandsonTableHelperObjImgs[i] = muban_handsonTableImgs[handsonTableSelected][i];
        }
    }
    /*var newHandsonTableHelperObjParetoSettings = {};
    for (var i in handsonTableHelperObjParetoSettings) {
    ...
    handsonTableHelperObjBoxLineChartSettings = newHandsonTableHelperObjBoxLineChartSettings;
    handsonTableHelperObjParetoSettings = newHandsonTableHelperObjParetoSettings;*/
    muban_handsonTableImgs[handsonTableSelected] = newHandsonTableHelperObjImgs;
    /*handsonTableHelperObjControlCharts = newHandsonTableHelperObjControlCharts;
    handsonTableHelperObjBoxLineCharts = newHandsonTableHelperObjBoxLineCharts;
    handsonTableHelperObjSigmaMaps = newHandsonTableHelperObjSigmaMaps;
    handsonTableHelperObjPieCharts = newHandsonTableHelperObjPieCharts;
    ...
    //localStorage.setItem('handsonTableHelperObj', JSON.stringify(handsonTableHelperObj));
    handsonTable2.updateSettings(handsonTableOptions2);
    ...
    setTimeout(function() {
        ...
        $("#handsonContainer" + handsonTableSelected).width(handsonTable2.table.clientWidth);
        $("#handsonContainer" + handsonTableSelected).height(handsonTable2.table.clientHeight);
    }, 100);
}

function deleteRowWrapper() {
    if (yemianOrMubanMode == 2) {
        deleteRowMuban();
        return false
    } else {
...
function deleteRowMuban() {
    ...
    var handsonTableOptions2 = handsonTable2.getSettings();
    var newRow = mergeCellHelper.topLeft.y;
    handsonTableOptions2.data.splice(newRow, 1);
    handsonTableOptions2.rowHeights.splice(newRow, 1);
    handsonTableOptions2.startRows--;
    /*var newHandsonTableHelperObj = {};
    for (var i in handsonTableHelperObj) {
        var helper = i.split(',');
    ...
    //test this now, 11:35 AM, 04/18/2020, then go to next deleteRow, insertCol and deleteCol...
    var newMergeCells = [];
    if (handsonTableOptions2.mergeCells) {
        for (var i = 0; i < handsonTableOptions2.mergeCells.length; i++) {
            if (handsonTableOptions2.mergeCells[i].row > newRow) {
                newMergeCells.push({
                    row: handsonTableOptions2.mergeCells[i].row - 1,
                    col: handsonTableOptions2.mergeCells[i].col,
                    rowspan: handsonTableOptions2.mergeCells[i].rowspan,
                    colspan: handsonTableOptions2.mergeCells[i].colspan
                });
            } else if (handsonTableOptions2.mergeCells[i].row + handsonTableOptions2.mergeCells[i].rowspan <= newRow) {
                newMergeCells.push(handsonTableOptions2.mergeCells[i]);
            }
        }
    }
    var newHandsonTableHelperObjImgs = {}
...
function InsertColumnMuban() {
    ...
    var newCol = mergeCellHelper.topLeft.x;
    handsonTableOptions2.colWidths.splice(newCol, 0, 20);
    handsonTableOptions2.width += 20;
    handsonTableOptions2.startCols++;
    handsonTableOptions2.columns.push({renderer: "html"});
    /*var newHandsonTableHelperObj = {};
    for (var i in handsonTableHelperObj) {
        var helper = i.split(',');
        if (helper[1] >= newCol) {
    ...
    if (handsonTableOptions2.mergeCells) {
        for (var i = 0; i < handsonTableOptions2.mergeCells.length; i++) {
            if (handsonTableOptions2.mergeCells[i].col >= newCol) {
                newMergeCells.push({
                    row: handsonTableOptions2.mergeCells[i].row,
                    col: handsonTableOptions2.mergeCells[i].col + 1,
                    rowspan: handsonTableOptions2.mergeCells[i].rowspan,
                    colspan: handsonTableOptions2.mergeCells[i].colspan
                });
            } else {
    ...
    for (var i in muban_handsonTableImgs[handsonTableSelected]) {
        var helper = i.split(',');
        if (helper[1] >= newCol) {
            handsonTableOptions2.data[helper[0]][Number(helper[1]) + 1] =
                handsonTableOptions2.data[helper[0]][helper[1]]
            handsonTableOptions2.data[helper[0]][helper[1]] = '';
            newHandsonTableHelperObjImgs[helper[0] + ',' + (Number(helper[1]) + 1)] =
                muban_handsonTableImgs[handsonTableSelected][i];
    ...
    handsonTableHelperObjBoxLineChartSettings = newHandsonTableHelperObjBoxLineChartSettings;
    handsonTableHelperObjParetoSettings = newHandsonTableHelperObjParetoSettings;*/
    handsonTableOptions2.mergeCells = newMergeCells;
    muban_handsonTableImgs[handsonTableSelected] = newHandsonTableHelperObjImgs;
    /*handsonTableHelperObjControlCharts = newHandsonTableHelperObjControlCharts;
    handsonTableHelperObjBoxLineCharts = newHandsonTableHelperObjBoxLineCharts;
    handsonTableHelperObjSigmaMaps = newHandsonTableHelperObjSigmaMaps;
    ...
    handsonTable2.render();
    //resizeCharts();
    resizeChartsMuban(handsonTable2, null, null);
...
function deleteColumnMuban() {
    ...
    var mergeCellHelper = fourCornersHelper(currSelection[0]);
    var handsonTableOptions2 = handsonTable2.getSettings();
    handsonTableOptions2.colWidths.splice(newCol, 1);
    handsonTableOptions2.columns.splice(0, 1);
    handsonTableOptions2.startCols--;
    var newCol = mergeCellHelper.topLeft.x;
    /*var newHandsonTableHelperObj = {};
    for (var i in handsonTableHelperObj) {
    ...
    if (handsonTableOptions2.mergeCells) {
        for (var i = 0; i < handsonTableOptions2.mergeCells.length; i++) {
            ...
            } else if (handsonTableOptions2.mergeCells[i].col < newCol &&
                handsonTableOptions2.mergeCells[i].col + handsonTableOptions2.mergeCells[i].colspan <= newCol) {
                newMergeCells.push(handsonTableOptions2.mergeCells[i]);
            }
        }
    }
    ...
    for (var i in muban_handsonTableImgs[handsonTableSelected]) {
        ...
        if (helper[1] > newCol) {
            ...
            handsonTable2.data[helper[0]][helper[1]] = '';
        } else {
            newHandsonTableHelperObjImgs[i] = muban_handsonTableImgs[handsonTableSelected][i];
        }
    }
    ...
    handsonTableHelperObjStackingMapCharts = newHandsonTableHelperObjStackingMapCharts;
    handsonTableHelperObjHistograms = newHandsonTableHelperObjHistograms;*/
    for (var i = 0; i < handsonTableOptions2.data.length; i++) {
        handsonTableOptions2.data[i].splice(newCol, 1);
    }
...
function handoverReport(val) {
    $.ajax({
        ...
        success: function (data) {
            ...
            var clientWidthHelper = 60//50;
            var clientHeightHelper = 60//50;
            if (handsonTableOptions.colWidths) {
                for (var i = 0; i < handsonTableOptions.colWidths.length; i++) {
                    ...
                    //console.log("colWidth " + i + " = " + handsonTableOptions.colWidths[i])
                    clientWidthHelper += handsonTableOptions.colWidths[i];
                }
            }

            if (handsonTableOptions.rowHeights) {
                for (var i = 0; i < handsonTableOptions.rowHeights.length; i++) {
                    ...
                    //console.log("rowHeight " + i + " = " + handsonTableOptions.rowHeights[i])
            ...
            handsonTableOptions = {
                ...
                rowHeaders: true,
                colHeaders: true,
                width: clientWidthHelper,/*typeof handsonTableOptions.clientWidth !== 'undefined' &&
                    handsonTableOptions.clientWidth && !isNaN(handsonTableOptions.clientWidth) ?
                    handsonTableOptions.clientWidth + 50:
                    clientWidthHelper,*/
                height: clientHeightHelper,/*typeof handsonTableOptions.clientHeight !== 'undefined' &&
...
function newStabilizer() {
    var wtHolder = $(handsonTable.table).parents('.wtHolder');
    var wtHider = $(handsonTable.table).parents('.wtHider');
    var hT_t_clientWidth = handsonTable.table.clientWidth + 10
    var hT_t_clientHeight = handsonTable.table.clientHeight + 10
    if (wtHolder.length > 0 && wtHider.length > 0) {
    ...
    $("#handsonContainer").width(hT_t_clientWidth);
    $("#handsonContainer").height(hT_t_clientHeight);
...
function initHandsonTable(handsonTable) {
    ...
    handsonTable.addHook('afterSelection', function(row, column, row2, column2, preventScrolling, selectionLayerLevel) {
        ...
        if (handsonTableHelperObj && handsonTableHelperObj[row2+','+column2]) {
            echoCellOption(handsonTableHelperObj[row2 + ',' + column2])
        }
 ...
 * 回显单元格属性
 * @author nikoohp
 * @param {Object}} option 单元格属性
 */
function echoCellOption(option) {

    var rules = option.rules;
    var html = ''

    rules.forEach(function(rule) {
        var property = rule.cellProperties[0];

        html += `
            <div class="cellAttribute">
                <div class="bigAttribute">
                    <div style="height:20px;">
                        ...
                        <span onclick="NewlyAddedCommand(this)" class="element elxinjian1 fr NewlyAddedCommand MarginR10"></span>
                        <span onclick="DeleteAttributeCommand(this)" class="DeleteAttribute element elshanchu11 fr MarginR10"></span>
                        <p class="fr MarginR10 AddCommandText" >添加多条命令</p>
                    </div>
                    <div id="AddingConditions" class="AddingConditions">
                        <ul>
                            <li>
                                <label>命令</label>
                                <select id="cellProperties" class="cellProperties" value="${property.cellProperties}">
                            ...
                            <li>
                                <label>范围</label>
                                <select style="max-width: 60px;" class="RangeSymbol" value="${property.RangeSymbol}">
                                    <option value="greaterThan"> > </option>
                                    <option value="lessThan"> < </option>
                                    <option value="beEqualTo"> = </option>
                                </select>
                                <input type="number" class="RangeNumber" style="max-width: 100px;" value="${property.RangeNumber}" />
                            </li>
                        </ul>
                    </div>
                </div>
                <ul id="BackgroundColorGroup" class="BackgroundColorGroup">
                    <li>
                        <label><span style="vertical-align: top;">底色</span></label>
                        <div style="" class="fl BGCDiv">
    ...
    $(".tupianCont.publicStyle").find(".bigCellAttribute").append(html)


    rules.forEach(function (rule, idx) {
        var property = rule.cellProperties[0];
        console.log("echoCellOption -> property", property)

        $('.cellProperties').eq(idx).val(property.cellProperties)
        $('.RangeSymbol').eq(idx).val(property.RangeSymbol)
        //底色调用
        $(".cellAttribute").eq(idx).find(".BackgroundColor").colorpicker({
