(function($) {
	$.fn.extend({
		wybExcel: function(options) {
			...
			return this.each(function() {
				...
				if (op.data) {
					...
					initTable(t, {
						row: parseInt(op.row)+1,
						col: parseInt(op.col)+1,
						width: 0,
						type: 1
					})
				}
		...
		getExcelHtml: function() {
			...
			if (table.length == 1) {
				...
				clone.find("td").removeClass("td-position-css").removeClass("td-chosen-css").removeClass("td-chosen-muli-css").removeAttr(
					"contenteditable");
	...
	function drugCell(table, t) {
		$(table).find("tbody").removeClass("AutoWidth")
	...
	function drawChosenArea(table, t) {
		...
		if (coll.length > 0) {
			...
			$("<div class='chosen-area-p'></div>").width(1).height(height + 1).css("margin-top", first[0].offsetTop - 1).css(
				"margin-left", first[0].offsetLeft - 1).insertBefore(table);
			$("<div class='chosen-area-p'></div>").width(width + 1).height(1).css("margin-top", first[0].offsetTop - 1).css(
				"margin-left", first[0].offsetLeft - 1).insertBefore(table);
			$("<div class='chosen-area-p'></div>").width(1).height(height - 4).css("margin-top", first[0].offsetTop - 1).css(
				"margin-left", first[0].offsetLeft + width - 1).insertBefore(table);
			$("<div class='chosen-area-p'></div>").width(width - 4).height(1).css("margin-top", first[0].offsetTop + height - 1)
				.css("margin-left", first[0].offsetLeft - 1).insertBefore(table);
			...
			}).width(3).height(3).css("padding", "2px").css("margin-top", first[0].offsetTop + height - 3).css("margin-left",
				first[0].offsetLeft + width - 3).insertBefore(table)
		}
	}
	...
	function showRightPanel(table, t, e) {
		var coll = table.find(".td-chosen-css");
		closeRightPanel(t);
		var rightMousePanel = $("<div class='rightmouse-panel-div'></div>").css("left", e.clientX-300).css("top", e.clientY).insertBefore(
			table);
		var leftPanel = $("<div class='panel-div-left'></div>").width(150).appendTo(rightMousePanel);
		var rightPanel = $("<div class='panel-div-right'></div>").width(100).appendTo(rightMousePanel);
		...
		rightMousePanel.find(".wb").click(function() {
			...
			if (!obj.hasClass("duiqifangsi") && !obj.hasClass("setting")) {
				...
				if (obj.hasClass("chuizhijuzhong")) {
					coll.addClass("verticalMiddle")
				}
				if (obj.hasClass("dingduanduiqi")) {
					coll.addClass("verticalTop")
				}
				if (obj.hasClass("dibuduiqi")) {
					coll.addClass("verticalBottom")
				}
